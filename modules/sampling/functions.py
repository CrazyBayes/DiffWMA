...
from modules.diffusion.helpers import (
    extract,
    apply_conditioning,
)
import numpy as np
...
def default_sample_fn(
    model, x, cond, t, q_sample, condition_noise_scale, policy, normalizer
):
    timesteps = make_timesteps(x.shape[0], t, x.device)

    # rescale actions   ##q_sample是实现从x_0到x_t扩散过程
    act_scale = q_sample(
        timesteps, noise=condition_noise_scale
    )
